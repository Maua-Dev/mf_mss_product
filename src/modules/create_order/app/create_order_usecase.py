...
from src.shared.helpers.errors.usecase_errors import NoItemsFound, UnregisteredUser, \
    TimeReservedNeedsToBeAtLeastOneHourAhead, RestaurantDontPermitSchedule, TimeReservedNotAvailable, \
    MinimumPriceForTimeReserved
...
class CreateOrderUsecase:
    ...
    def __call__(self, user_name: str, user_id: str, products: List[OrderProduct],
                 restaurant: RESTAURANT, schedule: Optional[Schedule] = None) -> Order:
        ...
        if schedule:
            if self.repo_order.check_schedule(schedule):
        ...
        order = Order(order_id=order_id, user_name=user_name, user_id=user_id, products=products,
                      ...
                      action=ACTION.NEW, schedule=schedule)
