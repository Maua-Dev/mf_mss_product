...
class OrderRepositoryDynamo(IOrderRepository):
    ...
    def get_all_orders_by_user(self, user_id: str, exclusive_start_key: str or None, amount: int) -> List[Order]:
        # response = self.dynamo.scan_items(ExclusiveStartKey=exclusive_start_key, Limit=amount, filter_expression=Attr("restaurant").eq(restaurant)&Attr("entity").eq("order"))

        # all_orders_user = list()
        # for item in resp.get('Items'):
        #     if item.get('user_id') == user_id:
        #         all_orders_user.append(item)
    ...
    def get_all_orders_by_restaurant(self, restaurant: RESTAURANT, exclusive_start_key: str or None, amount: int) -> List[Order]:
        ...
        resp = self.dynamo.query(key_condition_expression=query_string, Select='ALL_ATTRIBUTES', ExclusiveStartKey=exclusive_start_key, Limit=amount)
