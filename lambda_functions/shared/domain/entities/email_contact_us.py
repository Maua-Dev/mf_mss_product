...
class EmailContactUs(Email):
    ...
    def __init__(self, to_address: str = None, message: str = None) -> None:
        ...
        if not self.validate_message(message)[0]:
            raise Exception(self.validate_message(message)[1])
        ...
        * {{
        ...
        """.format(message=self.body, date_time=self.date_time)

    @staticmethod
    def validate_message(message: str = None) -> Tuple[bool, str or None]:
        if message is None:
